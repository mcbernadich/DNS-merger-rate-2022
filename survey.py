...
class Survey:
    ...
    def __init__(self, surveyName, psr_name, corr_func=1, func1=1, func2=1, corr_norm=1, pattern='gaussian'):
        ...
        self.func = corr_func #correction function for intensity profile
        self.func1 = func1 #mp function
        self.func2 = func2 #ip function
        self.norm = corr_norm   #normalization of profile
    ...
    def SNRcalc(self,
        ...
        if self.inRegion(pulsar):
            ...
            if self.pointingslist is not None:
                ...
                #Repeat a calculation of an offset from a beam.
        else:
            return -2, 1, 1
        ...
        if self.psr_name=="1906":
            sig_to_noise = rad.calcSNR(self.calcflux_1906(pulsar, pop.ref_freq),
                                       self.beta,
                                       self.tsys,
                                       self.tskypy(pulsar),
                                       self.gain,
        ...
        return sig_to_noise, degfac, offset
    ...
    def calcflux_1906(self, psr, ref_freq):
        """Calculate the flux at this frequency"""
        
        #beta = np.degrees(np.arccos(random.uniform(0.,1.))) - 80.59 #calculate upper hemisphere and correct with fb=2
        beta = np.degrees(np.arccos(random.uniform(-1.,1.))) - 80.59 #calculate with both pulses
        
        if beta < -20. or beta > 34.:
            h = 0.
        else:
            h = np.exp(self.func(beta, self.func1, self.func2))/np.exp(self.norm)
        ...
        if psr.gpsFlag == 1:
            ...
            # assuming the alpha_1 value is for freq<ref_freq (which is ~1GHz)
            return psr.s_1400() * h * (self.freq / ref_freq)**psr.brokenSI

        else:
            return psr.s_1400() * h * (self.freq / ref_freq)**psr.spindex
